...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------- منوی اصلی ----------
    if text == "🛒 خرید کانفیگ":
        await show_plans(update, context, mode="buy")
...
async def show_plans(update, context, mode="buy"):
    conn = get_conn()
    plans = conn.execute("SELECT * FROM plans WHERE is_test=0").fetchall()
    conn.close()
    if not plans:
        await update.message.reply_text("در حال حاضر پلنی برای فروش ثبت نشده است.")
        return
    kb = []
    prefix = "buyplan" if mode == "buy" else "renewplan"
    for p in plans:
        cap = plan_capacity(p['id'])
        label = f"{p['title']} | {p['price']:,} تومان | موجودی: {cap}"
        kb.append([InlineKeyboardButton(label, callback_data=f"{prefix}_{p['id']}")])
    await update.message.reply_text("یکی از پلن‌ها رو انتخاب کنید:", reply_markup=InlineKeyboardMarkup(kb))
...
async def admin_edit_plan_start(update, context):
    ...
    conn.close()
    if not plans:
        await update.message.reply_text("هیچ پلنی ثبت نشده است.")
        return
    ...
    await update.message.reply_text("کدام پلن رو می‌خواهید ویرایش کنید؟", reply_markup=InlineKeyboardMarkup(kb))
...
async def admin_del_plan_start(update, context):
    ...
    kb.append([InlineKeyboardButton("❌ لغو", callback_data="cancel_action")])
    await update.message.reply_text("کدام پلن حذف شود؟", reply_markup=InlineKeyboardMarkup(kb))
...
async def handle_admin_text_states(update, context, state, text) -> bool:
    ...
    if state == "admin_new_plan_duration":
        if not text.isdigit():
            ...
            return True
        conn = get_conn()
        conn.execute("INSERT INTO plans(title, price, duration_days, is_test) VALUES (?,?,?,0)",
                     (d['title'], d['price'], int(text)))
        conn.commit()
        conn.close()
        clear_state(context)
        await update.message.reply_text(f"✅ پلن «{d['title']}» با قیمت {d['price']:,} تومان و {text} روز اعتبار ساخته شد.\nحالا از منوی ویرایش پلن، کانفیگ به آن اضافه کنید.")
        return True
